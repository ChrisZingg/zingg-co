...
<script>
// ── Vollständige Flugplatz-Datenbank ──
// Format: ICAO: [lat, lon, name, land, metar]  metar=true/false
const AIRPORTS = {
  // ── SCHWEIZ — Grosse Plätze ──
  ...
  LSZR: [47.542,  7.529,  'St.Gallen-Altenrhein',    'CH', true],
  ...
  LSMI: [46.676,  7.879,  'Interlaken',              'CH', false],
  ...
  EDFE: [49.960,  8.648,  'Frankfurt-Egelsbach',     'D',  false],
  EDDP: [51.432, 12.241,  'Leipzig/Halle',           'D',  true],
  ...
  LOKG: [47.440, 15.386,  'Kapfenberg',              'A',  false],
  LOAN: [47.843, 16.260,  'Wiener Neustadt',         'A',  false],
  LOLO: [47.617, 13.987,  'Schärding-Suben',         'A',  false],
  LOGK: [47.597, 14.908,  'Kapfenberg-Flugfeld',     'A',  false],
  LOXL: [48.234, 14.188,  'Linz-Blue Danube',        'A',  false],
  // ── FRANKREICH ──
  ...
  LFKJ: [41.924,  8.793,  'Ajaccio',                 'F',  true],
  ...
  LFMH: [45.540,  4.296,  'Saint-Etienne',           'F',  false],
  LFSN: [48.692,  6.230,  'Nancy-Essey',             'F',  false],
  LFST: [48.538,  7.628,  'Strasbourg',              'F',  true],
  // ── ITALIEN ──
  LIMC: [45.630,  8.723,  'Mailand-Malpensa',        'I',  true],
  LIME: [45.669,  9.704,  'Bergamo-Orio al Serio',   'I',  true],
  ...
  LIEE: [39.249,  9.054,  'Cagliari',                'I',  true],
  LIRA: [41.654, 12.590,  'Rom-Ciampino',            'I',  true],
  LIRF: [41.800, 12.239,  'Rom-Fiumicino',           'I',  true],
  LICJ: [38.176, 13.091,  'Palermo',                 'I',  true],
  LIMS: [44.924,  9.724,  'Piacenza',                'I',  false],
  LIPU: [45.396, 11.847,  'Padova',                  'I',  false],
  LIVD: [46.426, 12.582,  'Cortina/Fiames',          'I',  false],
  // ── LIECHTENSTEIN / VORARLBERG ──
  LSXV: [47.140,  9.530,  'Balzers (FL)',            'FL', false],
};
...
function suggestWaypoints() {
  ...
  const CORRIDOR_NM = totalDist > 300 ? 35 : 25;
  ...
  for (const [icao, data] of Object.entries(AIRPORTS)) {
    ...
    if (xtd <= CORRIDOR_NM && frac > 0.08 && frac < 0.92) {
  ...
  suggestions.forEach(s => {
    ...
    const metarTag = s.metar ? '' : ' <span style="font-size:9px;color:var(--muted);background:#eee;padding:1px 4px;border-radius:2px;">kein METAR</span>';
    ...
    btn.innerHTML = `<strong>${s.icao}</strong> <span style="color:var(--muted);font-size:10px;">${s.name} (${s.land}) · ${Math.round(s.xtd)} NM</span>${metarTag}`;
...
function renderWaypoints() {
  ...
  waypoints.forEach(icao => {
    ...
    const noMetar = ap && !ap[4] ? ' <span style="font-size:9px;color:var(--muted);">∅</span>' : '';
    span.innerHTML = icao + name + noMetar + `<button onclick="removeWaypoint('${icao}')" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:13px;padding:0 0 0 4px;line-height:1;">×</button>`;
...
async function fetchMetarForIcao(icao) {
  const ap = AIRPORTS[icao];
  if (ap && !ap[4]) return null; // kein METAR für diesen Platz
  try {
...
function metarToHtml(icao, raw, index, total) {
  ...
  if (!raw) return `<div class="card" style="border-left:3px solid var(--muted);">
    ...
    <div style="font-family:var(--mono);font-size:18px;font-weight:700;color:var(--sky);">${icao} <span style="font-size:12px;font-weight:400;color:var(--muted);">${apName}</span></div>
    <div style="margin-top:0.5rem;font-size:12px;color:var(--muted);font-family:var(--mono);">${noMetarExpected ? 'ℹ Kein METAR — Gras-/Kleinflugplatz' : '⚠ Kein METAR verfügbar'}</div>
  </div>`;
